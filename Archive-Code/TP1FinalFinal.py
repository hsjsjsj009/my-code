...
title("                                                                                     Colorful Chessboard and Flowers") #untuk 
screensize(canvwidth=7000, canvheight=7000, bg="black") #untuk mengatur ukuran canvas
...
if rows % 2 == 0 : #untuk mengecek barisnya genap atau ganjil
        x = -((rows)/2)*pixel 
        y = ((rows-2)/2)*pixel
        penup() #untuk membuat saat turtle berpindah tidak mencetak garis
        goto (x,y) #untuk menempatkan turtle
else :
        x = -((rows+1)/2)*pixel + (pixel//2)
        y = ((((rows+1)-2)/2)*pixel) - (pixel//2)
        penup()
        goto (x,y)
